...
def extract_date(text: str) -> str:
    ...
    print("=== priority_keywords results ===")
    priority_keywords = [
        r"伝票日付[：:\s]*(\d{4})[年/\-](\d{1,2})[年/\-](\d{1,2})",
        r"請求日[：:\s]*(\d{4})[年/\-](\d{1,2})[年/\-](\d{1,2})",
        r"発行日[：:\s]*(\d{4})[年/\-](\d{1,2})[年/\-](\d{1,2})",
        r"日付[：:\s]*(\d{4})[年/\-](\d{1,2})[年/\-](\d{1,2})",
        r"伝票日付[：:\s]*(\d{2})[年/\-](\d{1,2})[年/\-](\d{1,2})",
        r"通知日[：:\s]*(\d{4})年(\d{1,2})月(\d{1,2})日",
    ]
    for p in priority_keywords:
        m = re.search(p, text)
        print(f"キーワード: {p[:30]}... → {m.group() if m else 'なし'}")
    print("=================================")
    priority_keywords = [
